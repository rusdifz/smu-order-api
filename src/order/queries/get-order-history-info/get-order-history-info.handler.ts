...
import { GetOrderHistoryInfoResult } from './get-order-history-info.result';

@QueryHandler(GetOrderHistoryInfoQuery)
export class GetOrderHistoryInfoHandler
  implements IQueryHandler<GetOrderHistoryInfoQuery, GetOrderHistoryInfoResult>
{
  constructor(
    @InjectPinoLogger(GetOrderHistoryInfoHandler.name)
  ...
  async execute(
    query: GetOrderHistoryInfoQuery,
  ): Promise<GetOrderHistoryInfoResult> {
    ...
    if (!orders) throw createBadRequestException('order-history-not-found');
    ...
    return new GetOrderHistoryInfoResult(orders);
  }
}
